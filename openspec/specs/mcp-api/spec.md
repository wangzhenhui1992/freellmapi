...
通过 MCP Streamable HTTP 协议在独立端口暴露 `add_provider_key` 和 `add_custom_model` 两个工具，支持 AI Agent 程序化管理 freellmapi 的 key 和模型。TBD
...
- **THEN** 系统创建 `api_keys` 行（platform=custom, base_url=http://localhost:11434/v1），创建 `models` 行（platform=custom, model_id=`<keyId>-qwen3:4b`, source='user'），并补齐 `fallback_config` 条目，返回 `{ keyId, modelDbId, platform: "custom", baseUrl, models: ["<keyId>-qwen3:4b"] }`
